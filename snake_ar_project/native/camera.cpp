...
Mat Camera::getFrame(int display_option)
{
    ...
    else if (display_option == DISPLAY_OPTION_PIXELSTRIP) {
        return first_pixel_strip;
    }
    else if (display_option == DISPLAY_OPTION_MARKER) {
        return first_marker;
    }
    else {
        throw std::runtime_error("Unable to open camera");
...
Mat Camera::getFrameIfNewer(unsigned long &current, int display_frame_option)
{
    ...
    else if (display_frame_option == DISPLAY_OPTION_PIXELSTRIP) {
        return first_pixel_strip;
    }
    else if (display_frame_option == DISPLAY_OPTION_MARKER) {
        return first_marker;
    }
    else {
        throw std::runtime_error("Unable to open camera");
...
void Camera::loop()
{
    while (true)
    {
        auto start = std::chrono::high_resolution_clock::now();
        {
            ...
            {
                this->processContour(approx_contours[i], &frame);
            }
...
// Added in Sheet 2 - Ex4 End *****************************************************************

// Added in Sheet 2 - Ex5 Start ***************************************************************
...
#if DRAW_CONTOUR
        std::vector<std::vector<Point>> cov, aprox;
        cov.emplace_back(contours[k]);
        aprox.emplace_back(approx_contour);
        if (approx_contour.size() > 1) {
            cv::drawContours(*frame_out, cov, -1, Scalar(0, 255, 0), 4, 1);
            cv::drawContours(*frame_out, aprox, -1, Scalar(255, 0, 0), 4, 1);
            continue;
        }
#endif // DRAW_CONTOUR
...
#if DRAW_RECTANGLE
        cv::rectangle(frame_out, r, QUADRILATERAL_COLOR, 4);
        continue;
...
void Camera::processContour(std::vector<Point> approx_contour, cv::Mat* frame_draw)
{
    // Added in sheet 4 Ex9 - Start *******************************************************************
    ...
    if (marker_id == MARKER_ID_UNDEFINED) return;
}
// Added in Sheet 2 - Ex5 End ***************************************************************
...
std::array<cv::Point2f, 4> Camera::calculateSubpixCorners(float subpix_line_params[16], cv::Mat* frame_draw)
{
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        circle(*frame_draw, point_draw, 5, CV_RGB(255, 255, 0), -1);  // Added in sheet 4 Ex9.c)
